...
$mutex = New-Object System.Threading.Mutex($false, "Global\CursorCaptureHelper")
if (-not $mutex.WaitOne(0)) {
  Write-Host "cursor-capture already running"
  exit 0
}
...
public static class CursorSwallowM {
  ...
  static IntPtr Hook(int nCode, IntPtr wParam, IntPtr lParam) {
    if (nCode >= 0) {
      ...
      if (info.vk != VK_M && !IsCtrl(info.vk)) return CallNextHookEx(_hook, nCode, wParam, lParam);
      ...
      if (!_realCtrl && info.vk != VK_M) _seenM = false;
    }
    return CallNextHookEx(_hook, nCode, wParam, lParam);
  }
  public static void Pass(bool on) { _pass = on; }
  public static bool RealChord() { return _realCtrl && _realM; }
...
[CursorSwallowM]::Start()

$mouseSrc = @"
using System;
using System.Diagnostics;
using System.Runtime.InteropServices;
public static class CursorMouseBox {
...
[CursorMouseBox]::Start()

$code = @"
using System;
using System.Drawing;
using System.Windows.Forms;
public class CursorLiveRect : Form {
...
$overlay = New-Object CursorLiveRect
...
$outDir = Join-Path $env:TEMP "cursor-crops"
...
function KeyDown([int]$vk) { (([CursorSwallowM]::GetAsyncKeyState($vk) -band 0x8000) -ne 0) }
function KeyDownCtrl { ((KeyDown 0x11) -or (KeyDown 0xA2) -or (KeyDown 0xA3)) }

if (-not ([System.Management.Automation.PSTypeName]"CursorFG.Win").Type) {
  Add-Type -Name Win -Namespace CursorFG -MemberDefinition '
...
function Get-ForegroundProcessName {
  $h = [CursorFG.Win]::GetForegroundWindow()
  [uint32]$pid = 0
  [void][CursorFG.Win]::GetWindowThreadProcessId($h, [ref]$pid)
  if ($pid -eq 0) { return "" }
  try { return (Get-Process -Id $pid -ErrorAction Stop).ProcessName } catch { return "" }
}
...
$timer.Interval = 30
$timer.Add_Tick({
  $chord = [bool][CursorSwallowM]::Chord -or (KeyDownCtrl -and (KeyDown 0x4D))
  if ($chord) {
  ...
  [CursorMouseBox]::Arm = [bool]($talk -or $chord)
  $script:x0 = [CursorMouseBox]::X0; $script:y0 = [CursorMouseBox]::Y0
  $script:x1 = [CursorMouseBox]::X1; $script:y1 = [CursorMouseBox]::Y1
  if ([CursorMouseBox]::Boxing) {
    $script:boxing = $true
    Update-LiveRect
  }
  if ([CursorMouseBox]::Done) {
    [CursorMouseBox]::Done = $false
    $script:boxing = $false
    $overlay.Hide()
    try {
  ...
  if (-not $chord -and $script:talking -and $heldMs -ge 250) {
    if ([CursorMouseBox]::Boxing -or $script:boxing) { return }
    $script:talking = $false
    $script:talkAt = $null
    if ((Get-ForegroundProcessName) -eq "Grok Bot") {
      Write-Last @{ chord="release"; skipped="grok-bot" }
      $script:pendingFile = $null
...
$ig.Clear([System.Drawing.Color]::FromArgb(80, 140, 220))
...
$ig.DrawString("C", $font, [System.Drawing.Brushes]::White, (New-Object System.Drawing.RectangleF 0, 0, 16, 16), $sf)
$ig.Dispose()
$tray = New-Object System.Windows.Forms.NotifyIcon
$tray.Icon = [System.Drawing.Icon]::FromHandle($bmp.GetHicon())
$tray.Text = "Cursor capture - click to quit"
$tray.Visible = $true
